...
unsigned int sbytes, sinode_bytes, scache_bytes, sdirectory_bytes,
    sdirectory_cache_bytes, suid_count, sguid_count, stotal_bytes,
    stotal_inode_bytes, stotal_directory_bytes, sinode_count, sfile_count,
    ssym_count, sdev_count, sdir_count, sfifo_count, ssock_count, sdup_files;
int sfragments;
int restore = 0;
...
extern int read_super(int fd, squashfs_super_block *sBlk, int *be,
                      char *source);
extern int read_filesystem(
    char *root_name, int fd, squashfs_super_block *sBlk, char **cinode_table,
    ...
    squashfs_uid *uids, unsigned short *uid_count, squashfs_uid *guids,
    unsigned short *guid_count, unsigned int *uncompressed_file,
...
struct file_info *
duplicate(unsigned char *(get_next_file_block)(struct duplicate_buffer_handle *,
                                               unsigned int),
          struct duplicate_buffer_handle *file_start, int bytes,
          unsigned int **block_list, int *start, int blocks,
          struct fragment **fragment, char *frag_data, int frag_bytes);

#define FALSE 0

#define MKINODE(A)                                                             \
  ((squashfs_inode)(((squashfs_inode)inode_bytes << 16) +                      \
                    (((char *)A) - data_cache)))
 ...
 */
static int special_file(char *filename, char **dir_name, struct stat *buf) {
  char *bname;
  static char dev[32];
  char typ;
  int maj, min;
...
squashfs_base_inode_header *get_inode(int req_size) {
  ...
  while (cache_bytes >= SQUASHFS_METADATA_SIZE) {
    if ((inode_size - inode_bytes) < ((SQUASHFS_METADATA_SIZE << 1)) + 2) {
      if ((inode_table = (char *)realloc(
               inode_table, inode_size + (SQUASHFS_METADATA_SIZE << 1) + 2)) ==
          NULL) {
        goto failed;
      }
    ...
    if (check_data)
      *((unsigned char *)(inode_table + inode_bytes + block_offset - 1)) =
  ...
  if (data_space < req_size) {
    int realloc_size = cache_size == 0 ? ((req_size + SQUASHFS_METADATA_SIZE) &
                                          ~(SQUASHFS_METADATA_SIZE - 1))
                                       : req_size - data_space;

    if ((data_cache = (char *)realloc(data_cache, cache_size + realloc_size)) ==
        NULL) {
      goto failed;
    }
...
unsigned int write_inodes() {
  ...
  while (cache_bytes) {
    if (inode_size - inode_bytes < ((SQUASHFS_METADATA_SIZE << 1) + 2)) {
      if ((inode_table = (char *)realloc(
               inode_table,
               inode_size + ((SQUASHFS_METADATA_SIZE << 1) + 2))) == NULL) {
        BAD_ERROR("Out of memory in inode table reallocation!\n");
      }
      inode_size += (SQUASHFS_METADATA_SIZE << 1) + 2;
    }
    avail_bytes = cache_bytes > SQUASHFS_METADATA_SIZE ? SQUASHFS_METADATA_SIZE
                                                       : cache_bytes;
...
unsigned int write_directories() {
  ...
  while (directory_cache_bytes) {
    if (directory_size - directory_bytes <
      ...
      if ((directory_table = (char *)realloc(
               directory_table,
    ...
    if (check_data)
      *((unsigned char *)(directory_table + directory_bytes + block_offset -
...
int create_inode(squashfs_inode *i_no, char *filename, int type, int byte_size,
                 squashfs_block start_block, unsigned int offset,
                 unsigned int *block_list, struct fragment *fragment,
                 struct cached_dir_index *index, unsigned int i_count,
                 unsigned int i_size) {
  struct stat buf;
  ...
  base->guid =
      get_guid((squashfs_uid)global_uid == -1 ? buf.st_uid : global_uid,
               (squashfs_uid)global_gid == -1 ? buf.st_gid : global_gid);
  ...
  if (type == SQUASHFS_FILE_TYPE) {
    ...
    TRACE("File inode, file_size %d, start_block %x, blocks %d, fragment %d, "
          "offset %d, size %d\n",
          byte_size, start_block, offset, fragment->index, fragment->offset,
          fragment->size);
    for (i = 0; i < offset; i++)
...
void init_dir(struct directory *dir) {
  ...
  dir->list = NULL;
  dir->pathname = NULL;
  dir->count = dir->current_count = dir->i_count = dir->byte_count =
      dir->i_size = 0;
  dir->dir_is_ldir = TRUE;
}

void add_dir(squashfs_inode inode, char *name, int type,
  ...
  if (dir->p + sizeof(squashfs_dir_entry) + size + 6 >= dir->buff + dir->size) {
    if ((buff = (char *)realloc(dir->buff,
                                dir->size += SQUASHFS_METADATA_SIZE)) == NULL) {
  ...
  if (dir->entry_count == 256 || start_block != dir->start_block ||
      ((dir->entry_count_p != NULL) &&
       ((dir->p + sizeof(squashfs_dir_entry) + size - dir->index_count_p) >
        SQUASHFS_METADATA_SIZE))) {
    if (dir->entry_count_p) {
      squashfs_dir_header dir_header;

      if ((dir->p + sizeof(squashfs_dir_entry) + size - dir->index_count_p) >
          SQUASHFS_METADATA_SIZE) {
        if (dir->i_count % I_COUNT_SIZE == 0)
          if ((dir->index = realloc(
                   dir->index, (dir->i_count + I_COUNT_SIZE) *
                                   sizeof(struct cached_dir_index))) == NULL)
...
int write_dir(squashfs_inode *inode, char *filename, struct directory *dir) {
  ...
  if (data_space < dir_size) {
    ...
    if ((directory_data_cache =
             (char *)realloc(directory_data_cache,
                             directory_cache_size + realloc_size)) == NULL) {
      goto failed;
    }
  ...
  while (1) {
    ...
    memcpy(directory_data_cache, directory_data_cache + SQUASHFS_METADATA_SIZE,
           directory_cache_bytes - SQUASHFS_METADATA_SIZE);
    directory_cache_bytes -= SQUASHFS_METADATA_SIZE;
  }

  if (dir->dir_is_ldir) {
    if (create_inode(inode, filename, SQUASHFS_LDIR_TYPE, dir_size,
                     directory_block, directory_offset, NULL, NULL, dir->index,
                     dir->i_count, dir->i_size) == FALSE)
      return FALSE;
  } else {
    if (create_inode(inode, filename, SQUASHFS_DIR_TYPE, dir_size,
                     directory_block, directory_offset, NULL, NULL, NULL, 0,
                     0) == FALSE)
      return FALSE;
  }
...
char *get_fragment(char *buffer, struct fragment *fragment) {
  ...
  if (SQUASHFS_COMPRESSED_BLOCK(disk_fragment->size)) {
    ...
    if ((res = uncompress(buffer, &bytes, (const char *)cbuffer, size)) !=
        Z_OK) {
      if (res == Z_MEM_ERROR)
...
void write_fragment() {
  ...
  if (fragments % FRAG_SIZE == 0)
    if ((fragment_table = (squashfs_fragment_entry *)realloc(
             fragment_table, (fragments + FRAG_SIZE) *
                                 sizeof(squashfs_fragment_entry))) == NULL)
      BAD_ERROR("Out of memory in fragment table\n");
  fragment_table[fragments].size =
      mangle(buffer, fragment_data, fragment_size, block_size, noF, 1);
...
unsigned int write_fragment_table() {
  ...
  for (i = 0; i < fragments; i++, p++) {
    TRACE("write_fragment_table: fragment %d, start_block %x, size %d\n", i,
          fragment_table[i].start_block, fragment_table[i].size);
    if (!swap)
  ...
  for (i = 0; i < meta_blocks; i++) {
    int avail_bytes = i == meta_blocks - 1 ? frag_bytes % SQUASHFS_METADATA_SIZE
                                           : SQUASHFS_METADATA_SIZE;
    c_byte = mangle(cbuffer + block_offset, buffer + i * SQUASHFS_METADATA_SIZE,
                    avail_bytes, SQUASHFS_METADATA_SIZE, noF, 0);
    if (!swap)
...
unsigned char *read_from_buffer(struct duplicate_buffer_handle *handle,
...
unsigned short get_checksum(
    unsigned char *(get_next_file_block)(struct duplicate_buffer_handle *,
                                         unsigned int),
    struct duplicate_buffer_handle *handle, int l) {
...
int cached_frag = -1;
void add_file(int start, int file_bytes, unsigned int *block_listp, int blocks,
              unsigned int fragment, int offset, int bytes) {
  struct fragment *frg;
  ...
  handle.start = start;
  if ((dupl_ptr = duplicate(read_from_file, &handle, file_bytes, &block_listp,
                            &start, blocks, &frg, datap, bytes)) != NULL)
...
duplicate(unsigned char *(get_next_file_block)(struct duplicate_buffer_handle *,
          ...
          struct fragment **fragment, char *frag_data, int frag_bytes) {
  unsigned short checksum =
      get_checksum(get_next_file_block, file_start, bytes);
  struct duplicate_buffer_handle handle = {frag_data, 0};
  unsigned short fragment_checksum =
      get_checksum(read_from_buffer, &handle, frag_bytes);
  struct file_info *dupl_ptr =
      bytes ? dupl[checksum] : frag_dups[fragment_checksum];

  for (; dupl_ptr; dupl_ptr = dupl_ptr->next)
    if (bytes == dupl_ptr->bytes && frag_bytes == dupl_ptr->fragment->size &&
      ...
      while (dup_bytes) {
        int avail_bytes = dup_bytes > SQUASHFS_FILE_MAX_SIZE
      ...
      if (dup_bytes == 0) {
        ...
        if (dupl_ptr->fragment->index == fragments ||
            dupl_ptr->fragment->index == SQUASHFS_INVALID_BLK)
        ...
        if (frag_bytes == 0 ||
            memcmp(frag_data, fragment_buffer1, frag_bytes) == 0) {
          TRACE("Found duplicate file, start 0x%x, size %d, checksum 0x%x, "
                "fragment %d, size %d, offset %d, checksum 0x%x\n",
                dupl_ptr->start, dupl_ptr->bytes, dupl_ptr->checksum,
                dupl_ptr->fragment->index, frag_bytes,
                dupl_ptr->fragment->offset, fragment_checksum);
  ...
  if ((dupl_ptr = (struct file_info *)malloc(sizeof(struct file_info))) ==
      NULL) {
  ...
  if ((dupl_ptr->block_list =
           (unsigned int *)malloc(blocks * sizeof(unsigned int))) == NULL) {
...
int write_file(squashfs_inode *inode, char *filename, long long size,
               int *duplicate_file) {
  unsigned int frag_bytes, file, start, file_bytes = 0, block = 0;
  unsigned int c_byte;
  long long read_size =
      (size > SQUASHFS_MAX_FILE_SIZE) ? SQUASHFS_MAX_FILE_SIZE : size;
  ...
  do {
    if ((c_buffer = (char *)malloc((allocated_blocks + 1) << block_log)) ==
        NULL) {
      TRACE("Out of memory allocating write_file buffer, allocated_blocks %d, "
            "blocks %d\n",
            allocated_blocks, blocks);
      whole_file = 0;
      if ((allocated_blocks << (block_log - 1)) < MINALLOCBYTES)
        BAD_ERROR("Out of memory allocating write_file buffer, could not "
                  "allocate %d blocks (%d Kbytes)\n",
  ...
  for (start = bytes; block < blocks; file_bytes += bbytes) {
    for (i = 0, bbytes = 0; (i < allocated_blocks) && (block < blocks); i++) {
      int available_bytes = read_size - (block * block_size) > block_size
                                ? block_size
                                : read_size - (block * block_size);
      if (read(file, buff, available_bytes) == -1)
        goto read_err;
      c_byte =
          mangle(c_buffer + bbytes, buff, available_bytes, block_size, noD, 1);
  ...
  if (whole_file) {
    ...
    if (duplicate_checking &&
        (dupl_ptr =
             duplicate(read_from_buffer, &handle, file_bytes, &block_listp,
                       &start, blocks, &fragment, buff, frag_bytes)) == NULL) {
      *duplicate_file = TRUE;
      goto wr_inode;
    }
    ...
    handle.start = start;
    if (duplicate_checking &&
        (dupl_ptr = duplicate(read_from_file, &handle, file_bytes, &block_listp,
                              &start, blocks, &fragment, buff, frag_bytes)) ==
            NULL) {
      bytes = start;
      if (!block_device)
        ftruncate(fd, bytes);
...
wr_inode:
  free(c_buffer);
  file_count++;
  return create_inode(inode, filename, SQUASHFS_FILE_TYPE, read_size, start,
                      blocks, block_listp, fragment, NULL, 0, 0);
...
void add_dir_entry(char *name, char *pathname, void *data,
                   struct directory *dir) {
  if ((dir->count % DIR_ENTRIES) == 0)
    if ((dir->list = realloc(dir->list, (dir->count + DIR_ENTRIES) *
                                            sizeof(struct dir_ent *))) == NULL)
...
void sort_directory(struct directory *dir) {
  ...
  if (filesystem_minor_version == 0 ||
      (dir->count < 257 && dir->byte_count < SQUASHFS_METADATA_SIZE))
...
int encomp_opendir(char *pathname, struct directory *dir) {
  ...
  for (i = 0; i < source; i++) {
    ...
    pass = 1;
    for (;;) {
      for (n = 0; n < dir->count && strcmp(dir->list[n]->name, dir_name) != 0;
           n++)
        ;
      if (n == dir->count)
        break;
...
int single_opendir(char *pathname, struct directory *dir) {
  ...
  while ((d_name = readdir(linuxdir)) != NULL) {
    ...
    pass = 1;
    for (;;) {
      for (i = 0; i < dir->count && strcmp(dir->list[i]->name, dir_name) != 0;
           i++)
        ;
      if (i == dir->count)
        break;
...
int linux_readdir(struct directory *dir, char **filename, char **dir_name) {
  ...
  while ((current_count = dir->current_count++) < dir->count)
    if (dir->list[current_count]->data)
      add_dir(dir->list[current_count]->data->inode,
              dir->list[current_count]->name,
              dir->list[current_count]->data->type, dir);
    else {
      if (dir->list[current_count]->pathname)
        *filename = dir->list[current_count]->pathname;
      else
        strcat(strcat(strcpy(*filename, dir->pathname), "/"),
               dir->list[current_count]->name);
...
int dir_scan(squashfs_inode *inode, char *pathname,
             int(_opendir)(char *, struct directory *)) {
  struct stat buf;
  ...
  while (linux_readdir(&dir, &filename, &dir_name) != FALSE) {
    ...
    case S_IFREG:
      ...
      if (!sorted) {
        ...
        INFO("file %s, uncompressed size %lld bytes, %s\n", filename,
             buf.st_size, duplicate_file ? "DUPLICATE" : "");
      } else
    ...
    case S_IFLNK:
      ...
      result = create_inode(inode, filename, squashfs_type, 0, 0, 0, NULL, NULL,
                            NULL, 0, 0);
...
#define ADD_ENTRY(buf)                                                         \
  if (exclude % EXCLUDE_SIZE == 0) {                                           \
    if ((exclude_paths = (struct exclude_info *)realloc(                       \
             exclude_paths,                                                    \
             (exclude + EXCLUDE_SIZE) * sizeof(struct exclude_info))) == NULL) \
...
void add_old_root_entry(char *name, squashfs_inode inode, int type) {
  if ((old_root_entry = (struct old_root_entry_info *)realloc(
           old_root_entry, sizeof(struct old_root_entry_info) *
...
#define VERSION()                                                              \
  ...
  printf(                                                                      \
      "copyright (C) 2005 Phillip Lougher (phillip@lougher.demon.co.uk)\n\n"); \
...
int main(int argc, char *argv[]) {
  ...
  for (; i < argc; i++) {
    if (strcmp(argv[i], "-b") == 0) {
      ...
      if ((block_log = slog(block_size)) == 0) {
        ERROR(
            "%s: -b block size not power of two or not between 4096 and 64K\n",
            argv[0]);
        exit(1);
      }
    ...
    printOptions:
      ERROR("SYNTAX:%s source1 source2 ...  dest [options] [-e list of "
            "exclude\ndirs/files]\n",
            argv[0]);
      ERROR("\nOptions are\n");
      ERROR("-version\t\tprint version, licence and copyright message\n");
      ERROR("-info\t\t\tprint files written to filesystem\n");
      ERROR("-b <block_size>\t\tset data block to <block_size>.  Default %d "
            "bytes\n",
            SQUASHFS_FILE_SIZE);
      ...
      ERROR("-always-use-fragments\tuse fragment blocks for files larger than "
            "block size\n");
      ERROR("-no-duplicates\t\tdo not perform duplicate checking\n");
      ERROR("-noappend\t\tdo not append to existing filesystem\n");
      ERROR("-keep-as-directory\tif one source directory is specified, create "
            "a root\n");
      ERROR("\t\t\tdirectory containing that directory, rather than the\n");
      ERROR("\t\t\tcontents of the directory\n");
      ERROR("-root-becomes <name>\twhen appending source files/directories, "
            "make the\n");
  ...
  if (stat(argv[source + 1], &buf) == -1) {
    if (errno == ENOENT) { /* Does not exist */
      if ((fd = open(argv[source + 1], O_CREAT | O_TRUNC | O_RDWR,
    ...
    if (S_ISBLK(buf.st_mode)) {
      ...
      if ((fd = open(argv[source + 1], (delete ? O_TRUNC : 0) | O_RDWR)) ==
          -1) {
    ...
    if (!delete) {
      if (read_super(fd, &sBlk, &orig_be, argv[source + 1]) == 0) {
        ERROR("Failed to read existing filesystem - will not overwrite - "
              "ABORTING!\n");
        EXIT_MKSQUASHFS();
      }
    }
  ...
  /* process the exclude files - must be done afer destination file has been
   * possibly created */
  ...
  if (delete) {
    printf("Creating %s %d.%d filesystem on %s, block size %d.\n",
           be ? "big endian" : "little endian", SQUASHFS_MAJOR,
           filesystem_minor_version, argv[source + 1], block_size);
    bytes = sizeof(squashfs_super_block);
  } else {
    unsigned int last_directory_block, inode_dir_offset, inode_dir_file_size,
        root_inode_size, inode_dir_start_block, uncompressed_data,
        compressed_data;
    unsigned root_inode_start = SQUASHFS_INODE_BLK(sBlk.root_inode),
    ...
    if ((bytes = read_filesystem(
             root_name, fd, &sBlk, &inode_table, &data_cache, &directory_table,
             &directory_data_cache, &last_directory_block, &inode_dir_offset,
             &inode_dir_file_size, &root_inode_size, &inode_dir_start_block,
             &file_count, &sym_count, &dev_count, &dir_count, &fifo_count,
             &sock_count, (squashfs_uid *)uids, &uid_count,
             (squashfs_uid *)guids, &guid_count, &total_bytes,
             &total_inode_bytes, &total_directory_bytes, add_old_root_entry,
             &fragment_table)) == 0) {
      ERROR("Failed to read existing filesystem - will not overwrite - "
            "ABORTING!\n");
      EXIT_MKSQUASHFS();
    ...
    if ((fragments = sBlk.fragments))
      fragment_table = (squashfs_fragment_entry *)realloc(
          (char *)fragment_table,
    ...
    printf("Appending to existing %s %d.%d filesystem on %s, block size %d\n",
           be ? "big endian" : "little endian", SQUASHFS_MAJOR,
           filesystem_minor_version, argv[source + 1], block_size);
    printf("All -be, -le, -b, -noI, -noD, -noF, -check_data, no-duplicates, "
           "no-fragments, -always-use-fragments and -2.0 options ignored\n");
    printf("\nIf appending is not wanted, please re-run with -noappend "
           "specified!\n\n");

    compressed_data =
        inode_dir_offset + inode_dir_file_size & ~(SQUASHFS_METADATA_SIZE - 1);
    uncompressed_data =
        inode_dir_offset + inode_dir_file_size & (SQUASHFS_METADATA_SIZE - 1);
    ...
    sdata_cache =
        (char *)malloc(scache_bytes = root_inode_offset + root_inode_size);
    sdirectory_data_cache =
        (char *)malloc(sdirectory_cache_bytes = uncompressed_data);
    memcpy(sdata_cache, data_cache, scache_bytes);
    memcpy(sdirectory_data_cache, directory_data_cache + compressed_data,
    ...
    /* set the filesystem state up to be able to append to the original
     * filesystem.  The filesystem state differs depending on whether we're
     * appending to the original root directory, or if the original root
     * directory becomes a sub-directory (root-becomes specified on command
     * line, here root_name != NULL)
     */
    ...
    if (root_name) {
      ...
      memmove(directory_data_cache, directory_data_cache + compressed_data,
              uncompressed_data);
    ...
    inode_count = file_count + dir_count + sym_count + dev_count + fifo_count +
                  sock_count;
  }
...
restore_filesystem:
  ...
  if (sBlk.no_uids = uid_count) {
    if (!swap)
      ...
      write_bytes(fd, bytes, uid_count * sizeof(squashfs_uid),
                  (char *)uids_copy);
    }
  ...
  if (sBlk.no_guids = guid_count) {
    if (!swap)
      ...
      SQUASHFS_SWAP_DATA(guids, guids_copy, guid_count,
                         sizeof(squashfs_uid) * 8);
      write_bytes(fd, bytes, guid_count * sizeof(squashfs_uid),
                  (char *)guids_copy);
    }
  ...
  if (!swap)
    write_bytes(fd, SQUASHFS_START, sizeof(squashfs_super_block),
                (char *)&sBlk);
  else {
  ...
  printf("\n%s filesystem, data block size %d, %s data, %s metadata, %s "
         "fragments\n",
         be ? "Big endian" : "Little endian", block_size,
         noD ? "uncompressed" : "compressed",
         noI ? "uncompressed" : "compressed",
         no_fragments ? "no"
         : noF        ? "uncompressed"
                      : "compressed");
