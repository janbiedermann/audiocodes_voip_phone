...
int read_block(long long start, long long *next, char *block,
  ...
  } else if (read_bytes(start, 2, (char *)&c_byte) == FALSE)
    goto failed;
...
int set_attributes(char *pathname, unsigned int mode, unsigned int uid,
                   unsigned int guid, unsigned int mtime,
  ...
  if (geteuid() == 0) {
    ...
    uid_t guid_value =
        guid == SQUASHFS_GUIDS ? uid_value : (uid_t)guid_table[guid];
...
char *read_fragment(unsigned int fragment) {
  ...
  if (cached_frag == SQUASHFS_INVALID_FRAG || fragment != cached_frag) {
    ...
    if (read_data_block(fragment_entry->start_block, fragment_entry->size,
                        fragment_data) == 0) {
...
int write_file(char *pathname, unsigned int fragment, unsigned int frag_bytes,
               unsigned int offset, unsigned int blocks, long long start,
...
int create_inode(char *pathname, unsigned int start_block, unsigned int offset,
  ...
  if (swap) {
    ...
    SQUASHFS_SWAP_BASE_INODE_HEADER(&header.base, &sinode,
  ...
  /*
          if(created_inode[header.base.inode_number - 1]) {
                  ...
                  if(link(created_inode[header.base.inode_number - 1], pathname)
     == -1) { ERROR("create_inode: failed to create hardlink, because %s\n",
  ...
  */
  ...
  case SQUASHFS_FILE_TYPE: {
    ...
    if (write_file(pathname, inode->fragment, frag_bytes, offset, blocks, start,
      ...
      set_attributes(pathname, inode->mode, inode->uid, inode->guid,
                     inode->mtime, FALSE);
      file_count++;
    }
    break;
    ...
    /*		case SQUASHFS_LREG_TYPE: {
       ...
       &sinode); } else memcpy(inode, block_ptr, sizeof(*inode));

                            frag_bytes = inode->fragment ==
       SQUASHFS_INVALID_FRAG ? 0 : inode->file_size % sBlk->block_size; offset =
       inode->offset; blocks = inode->fragment == SQUASHFS_INVALID_FRAG ?
       (inode->file_size
                                    + sBlk->block_size - 1) >> sBlk->block_log :
       inode->file_size >> sBlk->block_log; start = inode->start_block;

                            TRACE("create_inode: regular file, file_size %lld,
       blocks %d\n", inode->file_size, blocks);

                            if(write_file(pathname, inode->fragment, frag_bytes,
       offset, blocks, start, block_ptr + sizeof(*inode), inode->mode)) {
                                    set_attributes(pathname, inode->mode,
       inode->uid, inode->guid, inode->mtime, FALSE); file_count ++;
                            }
                            break;
                    }
    */
  case SQUASHFS_SYMLINK_TYPE: {
    ...
    strncpy(name, block_ptr + sizeof(squashfs_symlink_inode_header),
            inodep->symlink_size);
    ...
    if (geteuid() == 0) {
      ...
      uid_t guid_value = inodep->guid == SQUASHFS_GUIDS
                             ? uid_value
                             : (uid_t)guid_table[inodep->guid];
  ...
  case SQUASHFS_CHRDEV_TYPE: {
    ...
    if (geteuid() == 0) {
      if (mknod(pathname,
                inodep->inode_type == SQUASHFS_CHRDEV_TYPE ? S_IFCHR : S_IFBLK,
                makedev((inodep->rdev >> 8) & 0xff, inodep->rdev & 0xff)) ==
          -1) {
        ERROR("create_inode: failed to create %s device %s, because %s\n",
              inodep->inode_type == SQUASHFS_CHRDEV_TYPE ? "character"
                                                         : "block",
              pathname, strerror(errno));
        break;
      }
      set_attributes(pathname, inodep->mode, inodep->uid, inodep->guid,
                     0 /*inodep->mtime todo: fix proper*/, TRUE);
      dev_count++;
    } else
      ERROR("create_inode: could not create %s device %s, because you're not "
            "superuser!\n",
  ...
  case SQUASHFS_FIFO_TYPE:
    ...
    set_attributes(pathname, header.base.mode, header.base.uid,
                   header.base.guid, 0 /* header.base.mtime todo */, TRUE);
    fifo_count++;
    break;
...
void uncompress_directory_table(long long start, long long end,
  ...
  while (start < end) {
    if (size - bytes < SQUASHFS_METADATA_SIZE &&
        (directory_table =
             realloc(directory_table, size += SQUASHFS_METADATA_SIZE)) == NULL)
...
struct dir *squashfs_openddir(unsigned int block_start, unsigned int offset,
  ...
  while (bytes < size) {
    ...
    TRACE("squashfs_opendir: Read directory header @ byte position %d, %d "
          "directory entries\n",
          bytes, dir_count);
    bytes += sizeof(dirh);
    ...
    while (dir_count--) {
      ...
      if ((dir->dir_count % DIR_ENT_SIZE) == 0) {
        if ((new_dir = realloc(dir->dirs, (dir->dir_count + DIR_ENT_SIZE) *
...
int dir_scan(char *parent_name, unsigned int start_block, unsigned int offset,
  ...
  !lsonly &&set_attributes(parent_name, dir->mode, dir->uid, dir->guid,
                           dir->mtime, TRUE);
...
int read_super(squashfs_super_block *sBlk, char *source) {
  ...
  if (sBlk->s_major != SQUASHFS_MAJOR || sBlk->s_minor > SQUASHFS_MINOR) {
    ...
    ERROR("I only support Squashfs 3.0 filesystems!  Later releases will "
          "support older Squashfs filesystems\n");
    goto failed_mount;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (i == argc) {
    ...
    options:
      ...
      ERROR("\t-dest <pathname>\tunsquash to <pathname>, default "
            "\"squashfs-root\"\n");
    }
    exit(1);
  }
  ...
  uncompress_directory_table(sBlk.directory_table_start,
                             sBlk.fragment_table_start, &sBlk);

  dir_scan(dest, SQUASHFS_INODE_BLK(sBlk.root_inode),
           SQUASHFS_INODE_OFFSET(sBlk.root_inode), &sBlk);
