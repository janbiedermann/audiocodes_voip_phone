...
int read_block(int fd, int start, int *next, unsigned char *block,
  ...
  if (SQUASHFS_COMPRESSED(c_byte)) {
    ...
    if ((res = uncompress(block, &bytes, (const char *)buffer, c_byte)) !=
        Z_OK) {
      if (res == Z_MEM_ERROR)
...
int scan_inode_table(
    int fd, int start, int end, int root_inode_start, int root_inode_offset,
    squashfs_super_block *sBlk, squashfs_inode_header *dir_inode,
    unsigned char **inode_table, unsigned int *root_inode_block,
    unsigned int *root_inode_size, unsigned int *uncompressed_file,
  ...
  while (start < end) {
    if (start == root_inode_start) {
      TRACE("scan_inode_table: read compressed block 0x%x containing root "
            "inode\n",
            start);
    ...
    if ((size - bytes < SQUASHFS_METADATA_SIZE) &&
        ((*inode_table =
              realloc(*inode_table, size += SQUASHFS_METADATA_SIZE)) == NULL))
      return FALSE;
    TRACE("scan_inode_table: reading block 0x%x\n", start);
    if ((bytes += read_block(fd, start, &start, *inode_table + bytes, sBlk)) ==
        0) {
      free(*inode_table);
      return FALSE;
    }
   ...
   * last directory start block index.  This is used when calculating the total
   * uncompressed directory size.  The directory bytes in the last block will be
   * counted as normal.
   *
  ...
  *root_inode_size = bytes - (*root_inode_block + root_inode_offset);
  ...
  for (cur_ptr = *inode_table; cur_ptr < *inode_table + bytes; files++) {
    ...
    TRACE(
        "scan_inode_table: processing inode @ byte position 0x%x, type 0x%x\n",
        cur_ptr - *inode_table, inode.inode_type);
    switch (inode.inode_type) {
    case SQUASHFS_FILE_TYPE: {
      int frag_bytes = inode.fragment == SQUASHFS_INVALID_BLK
                           ? 0
                           : inode.file_size % sBlk->block_size;
      int blocks =
          inode.fragment == SQUASHFS_INVALID_BLK
              ? (inode.file_size + sBlk->block_size - 1) >> sBlk->block_log
      ...
      *uncompressed_file += inode.file_size;
      ...
      add_file(start, file_bytes, block_list, blocks, inode.fragment,
...
int read_super(int fd, squashfs_super_block *sBlk, int *be, char *source) {
  ...
  if (sBlk->s_magic != SQUASHFS_MAGIC) {
    if (sBlk->s_magic == SQUASHFS_MAGIC_SWAP) {
      squashfs_super_block sblk;
      ERROR("Reading a different endian SQUASHFS filesystem on %s - ignoring "
            "-le/-be options\n",
            source);
  ...
  if (sBlk->s_major != SQUASHFS_MAJOR || sBlk->s_minor > SQUASHFS_MINOR) {
    if (sBlk->s_major == 1)
      ERROR("Filesystem on %s is a SQUASHFS 1.x filesystem.  Appending\nto "
            "SQUASHFS 1.x filesystems is not supported.  Please convert it\nto "
            "a SQUASHFS 2.1 filesystem...\n",
            source);
    else
      ERROR("Major/Minor mismatch, filesystem on %s is (%d:%d), I support (%d: "
            "<= %d)\n",
            source, sBlk->s_major, sBlk->s_minor, SQUASHFS_MAJOR,
            SQUASHFS_MINOR);
    goto failed_mount;
  }
...
unsigned char *
squashfs_readdir(int fd, int root_entries, int start, int offset, int size,
                 int *last_directory_block, squashfs_super_block *sBlk,
  ...
  while (bytes < size) {
    ...
    *last_directory_block = start;
    if ((bytes +=
         read_block(fd, start, &start, directory_table + bytes, sBlk)) == 0) {
  ...
  while (bytes < size) {
    ...
    TRACE("squashfs_readdir: Read directory header @ byte position 0x%x, 0x%x "
          "directory entries\n",
          bytes, dir_count);
    bytes += sizeof(dirh);
    ...
    while (dir_count--) {
      ...
      TRACE("squashfs_readdir: pushing directory entry %s, inode %x:%x, type "
            "0x%x\n",
...
int read_fragment_table(int fd, squashfs_super_block *sBlk,
  ...
  TRACE("read_fragment_table: %d fragments, reading %d fragment indexes from "
        "0x%x\n",
        sBlk->fragments, indexes, sBlk->fragment_table_start);
  if (sBlk->fragments == 0)
    return 1;

  if ((*fragment_table = (squashfs_fragment_entry *)malloc(
           sBlk->fragments * sizeof(squashfs_fragment_entry))) == NULL) {
  ...
  if (swap) {
    ...
    read_bytes(fd, sBlk->fragment_table_start,
               SQUASHFS_FRAGMENT_INDEX_BYTES(sBlk->fragments),
               (char *)sfragment_table_index);
    SQUASHFS_SWAP_FRAGMENT_INDEXES(fragment_table_index, sfragment_table_index,
                                   indexes);
  } else
    read_bytes(fd, sBlk->fragment_table_start,
               SQUASHFS_FRAGMENT_INDEX_BYTES(sBlk->fragments),
  ...
  for (i = 0; i < indexes; i++) {
    int length = read_block(
        fd, fragment_table_index[i], NULL,
...
int read_filesystem(
  ...
  unsigned int start = sBlk->inode_table_start,
               end = sBlk->directory_table_start,
  ...
  if ((files = scan_inode_table(
           fd, start, end, root_inode_start, root_inode_offset, sBlk, &inode,
           &inode_table, &root_inode_block, root_inode_size, uncompressed_file,
           uncompressed_directory, file_count, sym_count, dev_count, dir_count,
           fifo_count, sock_count)) == 0) {
  ...
  *uncompressed_inode = root_inode_block;
  ...
  if (inode.base.inode_type == SQUASHFS_DIR_TYPE ||
    ...
    if ((directory_table = squashfs_readdir(
             fd, !root_name,
             sBlk->directory_table_start + *inode_dir_start_block,
             *inode_dir_offset, *inode_dir_file_size, last_directory_block,
             sBlk, push_directory_entry)) == NULL) {
    ...
    if ((*cinode_table = (char *)malloc(root_inode_start)) == NULL) {
      ERROR("read_filesystem: failed to alloc space for existing filesystem "
            "inode table\n");
      goto error;
    }
    ...
    *last_directory_block -= sBlk->directory_table_start;
    ...
    read_bytes(fd, sBlk->directory_table_start, *last_directory_block,
               *cdirectory_table);

    if ((*data_cache = (char *)malloc(root_inode_offset + *root_inode_size)) ==
        NULL) {
    ...
    if ((*directory_data_cache = (char *)malloc(
             *inode_dir_offset + *inode_dir_file_size)) == NULL) {
    ...
    if (!swap)
      read_bytes(fd, sBlk->uid_start, sBlk->no_uids * sizeof(squashfs_uid),
                 (char *)uids);
    else {
      ...
      read_bytes(fd, sBlk->uid_start, sBlk->no_uids * sizeof(squashfs_uid),
                 (char *)uids_copy);
      SQUASHFS_SWAP_DATA(uids, uids_copy, sBlk->no_uids,
                         sizeof(squashfs_uid) * 8);
    }

    if (!swap)
      read_bytes(fd, sBlk->guid_start, sBlk->no_guids * sizeof(squashfs_uid),
                 (char *)guids);
    else {
      ...
      read_bytes(fd, sBlk->guid_start, sBlk->no_guids * sizeof(squashfs_uid),
                 (char *)guids_copy);
      SQUASHFS_SWAP_DATA(guids, guids_copy, sBlk->no_guids,
                         sizeof(squashfs_uid) * 8);
    }
